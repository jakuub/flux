...
class List extends Component<PersistentVector> {
  ...
  render() {
    return tiles.div(children: [tiles.div(children: "${countDone()} done issues"), renderTodos(), tiles.button(children: "remove done todos", listeners: {
        "onClick": removeDone
      })]);
  }
  ...
  renderTodo(PersistentMap todo, num index) {
    return tiles.li(key: todo.get("id"), children: [tiles.input(props: {
        "type": "checkbox"
      }..addAll(todo.get("done") ? {
            "checked": "checked"
          } : {}), listeners: {
